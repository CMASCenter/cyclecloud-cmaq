...
p1 <- ggplot(csv_data[!bad_rows,], aes(y=Scaling, x=CPUs, color=MS_Opt_Pin, size=ComputeNode)) +
    geom_point() + ggtitle("2 Day Benchmark Scaling versus CPU using HBv120") +  xlim(0,400) + ylim(0,55) + stat_smooth(method="lm")


p2 <- ggplot(csv_data[!bad_rows,], aes(y=Efficiency, x=CPUs, color=MS_Opt_Pin, size=ComputeNode)) +
    geom_point() + ggtitle("2 Day Benchmark Parallel Efficiency versus CPU using HBv120") + stat_smooth(method="lm")
