...
files   <- dir(sens.dir, pattern ='run_cctmv5.3.3_Bench_2016_12US2.576.24x24pe.2day.cyclecloud.*.codemod.nopin.redo.log')
files = rev(files)
#b.files <- dir(base.dir,pattern='run_cctmv5.3.3_Bench_2016_12US2.576.24x24pe.2day.cyclecloud.data.codemod.nopin.redo.log')
b.files <- dir(base.dir,pattern='run_cctmv5.3.3_Bench_2016_12US2.576.24x24pe.2day.cyclecloud.*.codemod.pin.ccc.log')
b.files = rev(b.files)
#Compilers <- c('intel','gcc','pgi')
Compilers <- c('gcc')
# name of the base case timing. I am using the current master branch from the CMAQ_Dev repository.
# The project directory name is used for the sensitivity case. 
base.name <- c('shared_pin','lustre_pin','data_pin')
#base.name <- c('data_pin')
sens.name <- c('shared_nopin','lustre_nopin','data_nopin')
