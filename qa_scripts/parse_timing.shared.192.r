...
files   <- dir(sens.dir, pattern ='run_cctmv5.3.3_Bench_2016_12US2.192.16x12pe.2day.cyclecloud.shared.codemod.pin.log')
#b.files <- dir(base.dir,pattern='run_cctmv5.3.3_Bench_2016_12US2.96.12x8pe.2day.cyclecloud.shared.codemod.nopin.redo.log')
b.files <- c('run_cctmv5.3.3_Bench_2016_12US2.192.16x12pe.2day.cyclecloud.data.codemod.pin.log','run_cctmv5.3.3_Bench_2016_12US2.192.16x12pe.2day.cyclecloud.lustre.codemod.pin.log')
...
# plot data
   ...
   png(file = paste(comp,'_192_',sens.name,'_',base.name,'.png',sep=''), width = 1024, height = 768, bg='white')
  # png(file = paste(comp,'_',sens.name,'.png',sep=''), width = 1024, height = 768, bg='white')
   barplot(bar.data, main = 'Process Timing on 192pes, 96x2, 16x12',names.arg = b.names,ylab='seconds', col = my.colors, legend = n.proc, xlim = c(0.,xmax),ylim = c(0.,5300.))
   box()
   dev.off()
