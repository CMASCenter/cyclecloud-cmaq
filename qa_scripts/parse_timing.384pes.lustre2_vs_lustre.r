...
files   <- dir(sens.dir, pattern ='run_cctmv5.3.3_Bench_2016_12US2.384.24x16pe.2day.cyclecloud.lustre.codemod.pin.log')
...
b.files <- c('run_cctmv5.3.3_Bench_2016_12US2.384.24x16pe.2day.cyclecloud.lustre2.codemod.pin.log')
Compilers <- c('gcc')
# name of the base case timing. I am using the current master branch from the CMAQ_Dev repository.
# The project directory name is used for the sensitivity case. 
base.name <- c('lustre2_384')
#base.name <- c('data_pin')
sens.name <- c('lustre_384')
...
# plot data
   ...
   png(file = paste('lustre2_vs_lustre_',comp,'_all',sens.name,'_',base.name,'.png',sep=''), width = 1024, height = 768, bg='white')
  # png(file = paste(comp,'_',sens.name,'.png',sep=''), width = 1024, height = 768, bg='white')
   barplot(bar.data, main = 'Process Timing on /lustre using 384 cpus/node on HBv3 120',names.arg = b.names,ylab='seconds', col = my.colors, legend = n.proc, xlim = c(0.,xmax),ylim = c(0.,3000.))
   box()
   dev.off()
