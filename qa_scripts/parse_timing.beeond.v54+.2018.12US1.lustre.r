...
files   <- dir(sens.dir, pattern ='run_cctm5.4plus_Bench_2018_12US1.96.12x8pe.2day.cyclecloud.lustre.3pull.log')
...
b.files <- c('run_cctm5.4plus_Bench_2018_12US1.192.16x12pe.2day.cyclecloud.lustre.log') 
...
# get timing info
	...
	# Timing <-  as.numeric(substr(data.in[grep('completed...',data.in)],42,48))
	 ...
	 dataout_timing <- as.numeric(substr(data.in[grep('Data Output completed...',data.in)],36,42))
   ...
   n.proc.plot <- str_trim(n.proc.plot, "left")
      ...
      # write csv
   write.csv(bar.data, "/shared/cyclecloud-cmaq/qa_scripts/lustre.csv")
   write.csv(n.proc.plot, "/shared/cyclecloud-cmaq/qa_scripts/lustre_proc.csv")
   write.csv(b.names, "/shared/cyclecloud-cmaq/qa_scripts/lustre_names.csv")
   ...
   png(file = paste('hb_120v3_hb176_12US1_CMAQv54+_lustre',comp,'.png',sep=''), width = 1024, height = 768, bg='white')
  # png(file = paste(comp,'_',sens.name,'.png',sep=''), width = 1024, height = 768, bg='white')
   barplot(bar.data, main = 'Process Timing CMAQv54+ 2018_12US1 on HB120v3',names.arg = b.names,ylab='seconds', col = my.colors, legend = n.proc.plot, xlim = c(0.,xmax),ylim = c(0.,8000.))
   box()
   dev.off()
