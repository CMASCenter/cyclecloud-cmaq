...
base.dir  <- '../run_scripts/HB120v3_pin_testing/lustre/logs/'
...
# get timing info
   ...
   pdf(file = paste('hb120v3_data_1-6nodes_',comp,'_all',sens.name,'_',base.name,'.pdf',sep=''), width = 9, height=7, bg='white')
  ...
  # png(file = paste(comp,'_',sens.name,'.png',sep=''), width = 1024, height = 768, bg='white')
   barplot(bar.data, main = 'Process Timing on /data using 1-6 nodes with 96 cpus/node on HB120v3 with pinning',names.arg = b.names,ylab='seconds', col = my.colors, legend = n.proc.plot, xlim = c(0.,xmax),ylim = c(0.,6000.))
   box()
   dev.off()
