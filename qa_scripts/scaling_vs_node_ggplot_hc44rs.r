...
png(file = paste('hc44rs','_','Scaling','_','Node','.png',sep=''), width = 1024, height = 768, bg='white')
...
p1 <- ggplot(csv_data, aes(y=Scaling, x=Nodes, color=COLROW, size=CPUs)) +
    geom_point() + ggtitle("2 Day Benchmark Scaling versus Nodes(18cpu/node) using HC44rs") + xlim(0,8) + ylim(0,16) 

p2 <- ggplot(csv_data, aes(y=Efficiency, x=Nodes, color=COLROW, size=CPUs)) +
    geom_point() + ggtitle("2 Day Benchmark Parallel Efficiency versus Node(18cpu/node) using HC44rs")

# Display both charts side by side thanks to the patchwork package
p1 + geom_abline(intercept = 0, slope = 2) + p2
