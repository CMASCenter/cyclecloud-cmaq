...
if [[ "$PLATFORM_ID" == "platform:el8" ]]; then
  wget -O /etc/yum.repos.d/beegfs_rhel8.repo https://www.beegfs.io/release/beegfs_7.2.12/dists/beegfs-rhel8.repo
elif [[ "$VERSION_ID" == "7" ]]; then
  wget -O /etc/yum.repos.d/beegfs_rhel7.repo https://www.beegfs.io/release/beegfs_7.2.12/dists/beegfs-rhel7.repo
else
...
if grep -q beeond-connauthfile /etc/beegfs/beegfs-meta.conf; then
  ...
  echo "sysMountSanityCheckMS=0" | tee -a /etc/beegfs/beegfs-client.conf
fi
EOF
...
yum install -y pdsh clustershell beeond libbeegfs-ib
...
wget -O /opt/beegfs/sbin/beeond https://raw.githubusercontent.com/themorey/cyclecloud-scripts/main/slurm-beeond/beeond-7212-cc
