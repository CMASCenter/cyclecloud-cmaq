...
setenv CMAQ_HOME /data-dec0104pl5app
setenv CMAQ_DATA /data-dec0104pl5app
setenv compilerString intel


#> Set General Parameters for Configuring the Simulation
 set VRSN      = v54p              #> Code Version
 set VRSN2      = v54             #> Code Version
 set PROC      = mpi               #> serial or mpi
 set MECH      = cracmm1_aq      #> Mechanism ID
## set APPL      = Bench_2018_12NE3  #> Application Name (e.g. Gridname)
 set APPL      = WR705_2018gc2_M3DRY_2018 #>(e.g. Gridname)
 set CASE      = 12US1             # jyun added for additional info for the output directory
                                                       
#> Define RUNID as any combination of parameters above or others. By default,
#> this information will be collected into this one string, $RUNID, for easy
#> referencing in output binaries and log files as well as in other scripts.
 setenv RUNID  ${VRSN}_intel_${MECH}_${APPL}

#> Set the build directory (this is where the CMAQ executable
#> is located by default).
 set BLD       = ${CMAQ_HOME}/CCTM/scripts/BLD_CCTM_${VRSN}_${compilerString}.cracmm
 set EXEC      = CCTM_${VRSN2}.exe  
...
## setenv OUTDIR  ${CMAQ_DATA}/output_CCTM_${RUNID}  #> Output Directory
 setenv OUTDIR  ${CMAQ_DATA}/output_CCTM_${RUNID}_${CASE}  #> Output Directory
## setenv INPDIR  ${CMAQ_DATA}/2018_12NE3            #> Input Directory
 setenv INPDIR  ${CMAQ_DATA}/inp_12US1/cctm_input    #> Input Directory
...
#> Set Start and End Days for looping
 ...
 set START_DATE = "2018-04-16"     #> beginning date (July 1, 2016)
 set END_DATE   = "2018-10-01"     #> ending date    (July 1, 2016)
...
if ( ! -e ${BLD}/CCTM_${VRSN2}.cfg ) then
   set SHAID = ""
else
   set SHAID = `grep "sha_ID" ${BLD}/CCTM_${VRSN2}.cfg | cut -c 13-22`
...
setenv GRIDDESC $INPDIR/GRIDDESC2    #> grid description file
...
#> Output Species and Layer Options
   ...
   setenv CONC_SPCS "O3 NO NO2 SO2 CO" 
...
set BCpath    = $INPDIR/icbc/CMAQv54_2018_108NHEMI_CRACCM1_FROM_CB6R5M_M3DRY     #> boundary conditions input directory
set EMISpath  = $INPDIR/emis/gridded_area           #> gridded emissions input directory
set EMISpath2 = $INPDIR/emis/gridded_area           #> gridded surface residential wood combustion emissions directory
set IN_PTpath = $INPDIR/emis/inln_point             #> point source emissions input directory
set IN_LTpath = $INPDIR/surface                     #> lightning NOx input directory
set METpath   = /data/temp/2018_12US1/met/WRFv4.3.3_LTNG_MCIP5.3.3  #> meteorology input directory
...
##     setenv ICFILE CCTM_ICON_v54_${MECH}_12NE3_20180701.nc
     setenv ICFILE CCTM_CGRID_v532_craccm1_aq_derived_from_cb6r3_ae7_aq_WR413_MYR_STAGE_2018_12US1_20180415.nc
...
##  set BCFILE = CCTM_BCON_v54_${MECH}_12NE3_${YYYYMMDD}.nc
  set BCFILE = BCON_CONC_12US1_CMAQv54_2018_108NHEMI_CRACCM1_FROM_CB6R5M_M3DRY_regrid_2018${MM}.nc
  ...
  setenv GRID_BDY_2D $METpath/GRIDBDY2D_${YYYYMMDD}.ncf  # GRID files are static, not day-specific
  setenv GRID_CRO_2D $METpath/GRIDCRO2D_${YYYYMMDD}.ncf
  setenv GRID_CRO_3D $METpath/GRIDCRO3D_${YYYYMMDD}.ncf
  setenv GRID_DOT_2D $METpath/GRIDDOT2D_${YYYYMMDD}.ncf
  setenv MET_CRO_2D $METpath/METCRO2D_${YYYYMMDD}.ncf
  setenv MET_CRO_3D $METpath/METCRO3D_${YYYYMMDD}.ncf
  setenv MET_DOT_3D $METpath/METDOT3D_${YYYYMMDD}.ncf
  setenv MET_BDY_3D $METpath/METBDY3D_${YYYYMMDD}.ncf
  setenv LUFRAC_CRO $METpath/LUFRAC_CRO_${YYYYMMDD}.ncf
...
##  setenv CMAQ_MASKS $INPDIR/GRIDMASK_STATES_12NE3.nc
  setenv CMAQ_MASKS $SZpath/OCEAN_${MM}_L3m_MC_CHL_chlor_a_12US1.ncf

  #> Gridded Emissions Files 

  setenv N_EMIS_GR 2 

  setenv SMOKEDIR /data/temp/2018_12US1/emis/WR705_2018gc2_cracmmv1/cmaq_ready/merged_nobeis_norwc  ##jyun added
  set EMISfile  = emis_mole_all_${YYYYMMDD}_12US1_nobeis_norwc_WR705_2018gc2.ncf
  cp ${SMOKEDIR}/${EMISfile}* ${EMISpath}
  setenv GR_EMIS_001 ${EMISpath}/${EMISfile}
  setenv GR_EMIS_LAB_001 GRIDDED_EMIS
  setenv GR_EM_SYM_DATE_001 F

  setenv SMOKEDIR2 /data/temp/2018_12US1/emis/WR705_2018gc2_cracmmv1/premerged/rwc
  set EMISfile  = emis_mole_rwc_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  cp ${SMOKEDIR2}/${EMISfile}* ${EMISpath2}
  setenv GR_EMIS_002 ${EMISpath2}/${EMISfile}
  setenv GR_EMIS_LAB_002 GR_RES_FIRES
  setenv GR_EM_SYM_DATE_002 F

#  setenv SMOKEDIR2 /data/2016platform/2016fj_16j/premerged/beis4
#  set EMISfile  = b4gts_l.2016fj_16j.${YYYYMMDD}.ncf
#  cp ${SMOKEDIR2}/${EMISfile}* ${EMISpath2}
#  setenv GR_EMIS_003 ${EMISpath2}/${EMISfile}
#  setenv GR_EMIS_LAB_003 GR_BEIS
#  setenv GR_EM_SYM_DATE_003 F

  #> In-line point emissions configuration
  setenv N_EMIS_PT 10          #> Number of elevated source groups

  set STKCASEG = 12US1_2016fj_16j           # Stack Group Version Label
  set STKCASEG2 = 12US2_2016fj_16j           # Stack Group Version Label
  set STKCASEE = 12US1_cmaq_cb6ae7_2016fj_16j  # Stack Emission Version Label
  set STKCASEE2 = 12US2_cmaq_cb6ae7_2016fj_16j  # Stack Emission Version Label

  # Time-Independent Stack Parameters for Inline Point Sources

  set IN_PTdir = /data/temp/2018_12US1/emis/WR705_2018gc2_cracmmv1/cmaq_ready
  set IN_PTdir2 = $INPDIR/emis/inln_point
  set IN_PTdir3 = /data/temp/2018_12US1/emis/WR705_2018gc2_cracmmv1/cmaq_ready

  setenv STK_GRPS_001 $IN_PTdir2/stack_groups.2018/stack_groups_ptnonipm_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_002 $IN_PTdir2/stack_groups.2018/stack_groups_ptegu_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_003 $IN_PTdir2/stack_groups.2018/stack_groups_othpt_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_004 $IN_PTdir2/stack_groups.2018/stack_groups_ptagfire_${YYYYMMDD}_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_005 $IN_PTdir2/stack_groups.2018/stack_groups_ptfire_grass_${YYYYMMDD}_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_006 $IN_PTdir2/stack_groups.2018/stack_groups_ptfire_othna_${YYYYMMDD}_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_007 $IN_PTdir2/stack_groups.2018/stack_groups_pt_oilgas_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_008 $IN_PTdir2/stack_groups.2018/stack_groups_cmv_c3_12_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_009 $IN_PTdir2/stack_groups.2018/stack_groups_cmv_c1c2_12_12US1_WR705_2018gc2.ncf
  setenv STK_GRPS_010 $IN_PTdir2/stack_groups.2018/stack_groups_ptfire_${YYYYMMDD}_12US1_WR705_2018gc2.ncf


  # Emission Rates for Inline Point Sources
        cp $IN_PTdir/othpt/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir3/ptagfire/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir/ptegu/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir3/ptfire_grass/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir3/ptfire/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir/ptfire_othna/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir/ptnonipm/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir/pt_oilgas/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir/cmv_c1c2_12/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        cp $IN_PTdir/cmv_c3_12/inln_mole*${YYYYMMDD}* ${IN_PTpath}
        gzip -d -f ${EMISpath}/*.gz
        gzip -d -f ${IN_PTpath}/*.gz
  setenv STK_EMIS_001 $IN_PTpath/inln_mole_ptnonipm_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_002 $IN_PTpath/inln_mole_ptegu_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_003 $IN_PTpath/inln_mole_othpt_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_004 $IN_PTpath/inln_mole_ptagfire_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_005 $IN_PTpath/inln_mole_ptfire_grass_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_006 $IN_PTpath/inln_mole_ptfire_othna_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_007 $IN_PTpath/inln_mole_pt_oilgas_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_008 $IN_PTpath/inln_mole_cmv_c3_12_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_009 $IN_PTpath/inln_mole_cmv_c1c2_12_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  setenv STK_EMIS_010 $IN_PTpath/inln_mole_ptfire_${YYYYMMDD}_12US1_cmaq_cracmmv1_WR705_2018gc2.ncf
  ...
  setenv STK_EMIS_LAB_010 PT_WFIRES
  ...
  setenv OCEAN_1 $SZpath/OCEAN_${MM}_L3m_MC_CHL_chlor_a_12US1.ncf #> horizontal grid-dependent ocean file

  #> Bidirectional ammonia configuration
  if ( $CTM_ABFLUX == 'Y' ) then
     setenv E2C_SOIL ${EPICpath}/2018r1_EPIC0509_12US1_soil.ncf
     setenv E2C_CHEM ${EPICpath}/2018r1_EPIC0509_12US1_time${YYYYMMDD}.ncf
     setenv E2C_CHEM_YEST ${EPICpath}/2018r1_EPIC0509_12US1_time${YESTERDAY}.ncf
     setenv E2C_LU ${EPICpath}/beld4_12US1_2011.ncf
  endif
...
#  ( /usr/bin/time -p mpirun -np $NPROCS $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
  ...
  set TASKMAP = ./machines216.air41230
  ( /usr/bin/time -p mpirun --bind-to core -machinefile $TASKMAP -np $NPROCS $BLD/$EXEC )|& tee buff_${EXECUTION_ID}.txt
  ...
  if ( ! -e $OUTDIR/CCTM_CGRID_${CTM_APPL}.nc ) then
    ...
    echo "**   repeat rather than proceeding to subsequent days.       **"
    echo "**************************************************************"
    set NDAYS = `echo "${NDAYS} - 1" | bc -l`
    goto repeatday
#    break
...
# ===================================================================
  ...
  #> remove emissions files :: jyun
  rm -f ${EMISpath}/*${YYYYMMDD}*
  rm -f ${EMISpath2}/*${YYYYMMDD}*
  rm -f ${IN_PTpath}/*${YYYYMMDD}*
  ...
  #> remove CCTM_CONC file
  rm -f $OUTDIR/CCTM_CONC_${CTM_APPL}.nc
